/* P0liped Firefox Detection 0.11 | GNU GPLv3 */
...
window.p0FFx = (function(document, window, undefined){ 
 ...
 // CSS' Appearance with "moz" prefix is available in all version starting from 1 -> https://developer.mozilla.org/en-US/docs/Web/CSS/-moz-appearance#Browser_compatibility
  ...
  var 
    createHTMLElement = function(){ // Function creating a HTML Element
      ...
      return function(tag){
        return document.createElement(typeof tag == "string" && !onlyLetters.test(tag) ? tag : "p0lip"); // Returns the created element you may work with
      };
    }(),
    ...
    */
    ...
    allTests = { // Each test returns a boolean. 
      // Most of features/bugs (also some other added features) may be found here https://developer.mozilla.org/en-US/Firefox/Releases/XX -> XX stands for version number.
      // To each test I added (or will add) some extra tests you may use, but remember to test them at your own. 
  ...
  // The Object.keys method is available in FF >= 4. We could assume the version number isn't higher than 4 (and iterate only last 4 tests), but this might be tricky, as someone might have overwritten Object.keys.
